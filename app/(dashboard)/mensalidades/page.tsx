...
interface ModalProps {
  aluno: Aluno;
  onClose: () => void;
  onSalvar: (alunoId: number, form: FormMensalidade) => void;
}

function ModalRegistrar({ aluno, onClose, onSalvar }: ModalProps) {
  const meses = [
    "Janeiro 2025",
    "Fevereiro 2025",
    "Março 2025",
  ...
  const [form, setForm] = useState<FormMensalidade>({
    mes: meses[6]!,
      ...
      <div className="bg-[#1a1a2e] border border-white/10 rounded-2xl w-full max-w-md shadow-2xl">
        <div className="px-6 pt-6 pb-4 border-b border-white/10 flex items-start justify-between">
          <div>
            ...
            <p className="text-sm text-white/40 mt-0.5">{aluno.nome}</p>
          </div>
...
export default function Mensalidades() {
  ...
  const [alunoAtivo, setAlunoAtivo] = useState<Aluno | null>(mockAlunos[0]!);
  ...
  const mensalidadesFiltradas = alunoAtivo
    ? filtroStatus === "todos"
      ...
      : alunoAtivo.mensalidades.filter((m) => m.status === filtroStatus)
    : [];

  const handleSalvar = (alunoId: number, form: FormMensalidade) => {
    setAlunos((prev) =>
      prev.map((a) => {
        ...
        return {
          ...a,
          mensalidades: [...a.mensalidades, nova],
        };
      }),
    );
  };
            ...
            {alunosFiltrados.map((aluno, i) => (
              ...
              >
                ...
                <div className="text-left">
                  ...
                  <p className="text-xs text-white/40">{aluno.modalidade}</p>
          ...
          {alunoAtivo && (
            <div className="bg-[#13131f] border border-white/10 rounded-2xl p-5 mb-5">
              <div className="flex items-center justify-between">
                <div>
                  <h2 className="text-2xl font-bold">{alunoAtivo.nome}</h2>

                  <p className="text-white/40 mt-1">{alunoAtivo.modalidade}</p>
                </div>

                <div className="bg-violet-500/10 border border-violet-500/20 px-4 py-2 rounded-xl">
                  <p className="text-xs text-violet-300 uppercase">Plano</p>
              ...
              >
                <div>
                  ...
                  <p className="text-sm text-white/40">
                    Vencimento:{" "}
                    {new Date(m.vencimento + "T00:00:00").toLocaleDateString(
                      "pt-BR",
                    )}
                  </p>
                </div>
                ...
                <div className="flex items-center gap-4">
                  <p className="font-bold">R$ {m.valor}</p>
